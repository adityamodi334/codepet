# this is the main python backend API code for the project!!!
...
def calc_xp(mins: int, mood: str) -> int:
    ...
    multipliers = {"flow": 1.5, "stuck": 1.3, "explore": 1.2, "grind": 1.0}
...
async def register(req: RegisterRequest, db: AsyncSession = Depends(get_db)):
    ...
    player = Player(username=req.username, pet_name=req.pet_name)
    db.add(player)
    await db.commit()
...
async def log_session(req: LogSessionRequest, db: AsyncSession = Depends(get_db)):
    ...
    if player.last_date != today:
        ...
        player.streak = player.streak + 1 if player.last_date == yesterday else 1
